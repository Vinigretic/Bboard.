...
def index(request):
    ...
    page = paginator_common(request, bbs)
...
def detail(request, rubric_pk, pk):
    ...
    if request.method == 'POST':
        ...
        if c_form.is_valid():
            ...
            form = c_form
            messages.add_message(request, messages.WARNING, 'Комментар не дододано')
...
def profile_bb_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            bb = form.save()
            formset = ALFormSet(request.POST, request.FILES, instance=bb)
            if formset.is_valid():
                formset.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if formset.is_valid():
                ...
                return redirect('main:profile_bb_detail', pk=pk)
    else:
        form = BbForm(instance=bb)
        formset = ALFormSet(instance=bb)
